...
`scripts/dump_densevlad_all_intermediate.m` (mode `densevlad`). It writes:
`./assets/torii15/matlab_dump/densevlad_dump_intermediate.mat`.

Tokyo247 golden references use:
`scripts/dump_densevlad_all_intermediate.m` (mode `tokyo247`), which writes:
...
`scripts/dump_densevlad_all_blackbox.m` with the same modes, which writes:
`./assets/torii15/matlab_dump/densevlad_dump_blackbox.mat`.
...
```
/Applications/MATLAB_R2025b.app/bin/matlab -batch "run('scripts/dump_densevlad_all_intermediate.m'); dump_densevlad_all_intermediate('densevlad')"
```
