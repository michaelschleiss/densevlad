...
`~/Library/Caches/densevlad/torii15/matlab_dump/densevlad_dump.mat`.

Masked grid parity uses:
`scripts/matlab/dump_densevlad_grid.m`, which writes:
`~/Library/Caches/densevlad/torii15/matlab_dump/densevlad_grid_dump.mat`.

Tokyo247 golden references use:
`scripts/matlab/dump_tokyo247_golden.m`, which writes:
- `~/Library/Caches/densevlad/torii15/matlab_dump/tokyo247_golden.mat`
- `~/Library/Caches/densevlad/torii15/matlab_dump/tokyo247_golden_list.txt`
...
`~/Library/Caches/densevlad/torii15/247code/data/dnscnt_RDSIFT_K128.cx_norm.npy`.
...
- The code downloads `247code.zip` from the authors to
  `~/Library/Caches/densevlad/torii15/247code.zip` on first use.
- Tests and scripts use `Torii15Assets.default()` to pull files from the zip
  into `~/Library/Caches/densevlad/torii15/247code/...`.
...
and extract into:
`~/Library/Caches/densevlad/torii15/tokyo247/database_gsv_vga/03814/...`
...
Extract to:
`~/Library/Caches/densevlad/torii15/247query_subset_v2/247query_subset_v2/`

3) **dbStruct metadata (tokyo247.mat)**
Download the NetVLAD-compatible dbStruct file:
```
curl -L -o ~/Library/Caches/densevlad/torii15/tokyo247/tokyo247.mat \
...
`~/Library/Caches/densevlad/torii15/tokyo247/densevlad_cache/`.
